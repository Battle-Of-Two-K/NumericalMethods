...
task = {
    'Функция для метода дихотоми': '0.5 * x * x * x + 6 * x - 2.5 ',
    'Отрезок для метода дихотомии': (0, 1),
    'Функция для метода хорд': '0.5 * x * x * x + 10 * x - 8',
    'Отрезок для метода хорд': (0, 1),
    'Функция для метода касательных': '0.5 * x *  x * x + 5 * x - 19.75',
    'Отрезок для метода касательных': (2, 3),
    'Функция для метода итераций': '4 * x * x * x + 5 * x + 14',
