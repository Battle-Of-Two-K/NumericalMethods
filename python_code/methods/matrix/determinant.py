...
def minor_method(matrix):
    ...
    if matrix.количество_строк <= 3:
        return diagonal_method(matrix)
    # Иначе - рекурсия
    else:
        # Но сначала, небольшой бонус от simens_green - ищем строку с наибольшим количеством нулей
        max_zeros_row_no = matrix.найти_максимальное_количество_вхождений(0)
        if matrix[max_zeros_row_no].count(0) == matrix.количество_столбцов:
            return 0
        det_value = 0
        for _ in range(matrix.количество_столбцов):
            ...
            if matrix[max_zeros_row_no][_] != 0:
                ...
                if bool((max_zeros_row_no + _ + 1) % 2):
                    det_value += matrix[max_zeros_row_no][_] * auto_det(matrix.удалить_строку_столбец(max_zeros_row_no, _))
                else:
                    det_value -= matrix[max_zeros_row_no][_] * auto_det(matrix.удалить_строку_столбец(max_zeros_row_no, _))
        return det_value
...
def diagonal_method(matrix):
    ...
    if not matrix.квадратная_ли:
        raise ArithmeticError("Определитель возможно найти только у квадратной матрицы")
    matrix = matrix.копия()
    if matrix.количество_строк == 1:
        return matrix[0][0]
    elif matrix.количество_строк == 2:
        return matrix[0][0] * matrix[1][1] - matrix[1][0] * matrix[0][1]
    elif matrix.количество_строк == 3:
